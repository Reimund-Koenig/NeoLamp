...
// fix crash on ESP32 (see
// https://github.com/alanswx/ESPAsyncWiFiManager/issues/44)
#if defined(ESP8266)
...
const char HTTP_STYLE[] PROGMEM =
    ...
    "line-height:2.4rem;font-size:1.2rem;width:100%;} .q{float: right;width: "
    "64px;text-align: right;} .l{background: "
    "url(\"data:image/"
...
const char HTTP_SCRIPT[] PROGMEM =
    ...
    "function f() {var x = "
    "document.getElementById('p');x.type==='password'?x.type='text':x.type='"
    "password';}"
    "</script>";
...
const char HTTP_PORTAL_OPTIONS[] PROGMEM =
    "<form action=\"/wifi\" method=\"get\"><button>Zur W-Lan "
    "Einstellung</button></form><br/>";
const char HTTP_PORTAL_OPTIONS_ORIGINAL[] PROGMEM =
...
const char HTTP_FORM_PARAM[] PROGMEM =
    "<br/><input id='{i}' name='{n}' length={l} placeholder='{p}' value='{v}' "
    "{c}>";
const char HTTP_FORM_END[] PROGMEM =
    "<br/><button type='submit'>Speichern und testen</button></form>";
const char HTTP_SCAN_LINK[] PROGMEM =
    "<br/><div class=\"c\"><a href=\"/wifi\">Suche vorhandene "
    "Netzwerke</a></div>";
const char HTTP_SAVED[] PROGMEM =
    "<div>W-Lan Zugangsdaten gespeichert.<br/><br/>Verbindung zum Netzwerk "
    "wird hergestellt....<br/></div>";
...
class AsyncWiFiManagerParameter {
  public:
    ...
    AsyncWiFiManagerParameter(const char *id, const char *placeholder,
                              const char *defaultValue, unsigned int length);
    AsyncWiFiManagerParameter(const char *id, const char *placeholder,
                              const char *defaultValue, unsigned int length,
                              const char *custom);
...
class AsyncWiFiManager {
  public:
    ...
    // if you want to always start the config portal, without trying to connect
    // first
    ...
    // sets timeout before webserver loop ends and exits even if there has been
    // no setup. usefully for devices that failed to connect at some point and
    // got stuck in a webserver loop. in seconds, setConfigPortalTimeout is a
    // new name for setTimeout
    void setConfigPortalTimeout(unsigned long seconds);
    void setTimeout(unsigned long seconds);

    // sets timeout for which to attempt connecting, usefull if you get a lot of
    // failed connects
    void setConnectTimeout(unsigned long seconds);

    // wether or not the wifi manager tries to connect to configured access
    // point even when configuration portal (ESP as access point) is running
    // [default true/on]
    ...
    // if this is set, it will exit after config, even if connection is
    // unsucessful
    void setBreakAfterConfig(boolean shouldBreak);
    // if this is set, try WPS setup when starting (this will delay config
    // portal for up to 2 mins)
    // TODO
  ...
  private:
    ...
    // const String  HTTP_HEAD = "<!DOCTYPE html><html lang=\"en\"><head><meta
    ...
    auto optionalIPFromString(...) -> bool {
        DEBUG_WM(F("NO fromString METHOD ON IPAddress, you need ESP8266 core "
                   "2.1.0 or newer for Custom IP configuration to work."));
        return false;
    }
};
